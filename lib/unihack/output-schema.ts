/**
 * Canonical UniHack Expected Output Schema
 * 
 * This file defines the exact 252-column delivery format from:
 * "Unihack_ Expected Output - Delivery Format.csv"
 * 
 * Headers are preserved exactly as they appear in the CSV,
 * including capitalization, spelling, and order.
 * 
 * Each header maps to an internal canonical field.
 * If a header has no internal mapping, it is marked as "null"
 * meaning it will always be empty unless explicitly populated.
 */

export const UNIHACK_HEADERS = [
  'MFR URL',
  'Ref URL 1',
  'Ref URL 2',
  'Ref URL 3',
...
export type UniHackHeader = typeof UNIHACK_HEADERS[number];

export const HEADER_TO_INTERNAL: Record<UniHackHeader, string | null> = {
  // Direct mappings from existing delivery format
  'MFR URL': 'null',
  'Ref URL 1': 'null',
  'Ref URL 2': 'null',
  'Ref URL 3': 'null',
  ...
  'Includes': 'null',
  'Product Name': 'null',
  // Attributes 1-50: ATTRIBUTE_LABEL 1-50, ATTRIBUTE_VALUE 1-50, ATTRIBUTE_UOM 1-50
...
export const INTERNAL_TO_HEADER: Record<string, UniHackHeader> = (() => {
  const map: Record<string, UniHackHeader> = {};
  for (const [header, internal] of Object.entries(HEADER_TO_INTERNAL)) {
    if (internal !== 'null' && internal !== undefined) {
      map[internal as string] = header as UniHackHeader;
    }
  }
  return map;
})();

export const TOTAL_HEADERS = UNIHACK_HEADERS.length; // 252
export const TOTAL_INTERNAL_FIELDS = Object.keys(HEADER_TO_INTERNAL).length;

// Validation: ensure no duplicate internal mappings that would cause column conflicts
export function validateHeaderOrder(headers: readonly UniHackHeader[]): boolean {
  if (headers.length !== TOTAL_HEADERS) return false;
  for (let i = 0; i < headers.length; i++) {
    if (headers[i] !== UNIHACK_HEADERS[i]) return false;
  }
  return true;
}

// Validation: ensure every header maps to exactly one internal field
export function validateMappingIntegrity(): boolean {
  const mapped = Object.values(HEADER_TO_INTERNAL).filter((v): v is string => v !== 'null');
  const unique = new Set(mapped);
  // Allow multiple headers to map to same internal field (e.g., PART_NUMBER, Mfg_Part_Num, SKU - MY_PART_NUMBER all -> mfg_part_num)
  // But no internal field should map to zero headers
  const headerCount = Object.keys(HEADER_TO_INTERNAL).length;
  return unique.size > 0 && headerCount === TOTAL_HEADERS;
}
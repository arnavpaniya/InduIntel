...
`CREATE INDEX IF NOT EXISTS idx_enrichment_cache_item_step ON enrichment_cache(item_id, step)`,
`ALTER TABLE enrichment_cache ENABLE ROW LEVEL SECURITY`,
`DROP POLICY IF EXISTS "Service role full access enrichment_cache" ON enrichment_cache`,
`CREATE POLICY "Service role full access enrichment_cache" ON enrichment_cache FOR ALL USING (auth.role() = 'service_role')`,
`DROP POLICY IF EXISTS "Public read enrichment_cache" ON enrichment_cache`,
...
async function main() {
  for (const stmt of statements) {
    ...
    if (error) {
      console.error('Error executing:', stmt.substring(0, 80), '...', error);
    } else {
